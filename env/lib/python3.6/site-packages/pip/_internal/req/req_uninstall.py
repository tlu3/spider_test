...
def uninstallation_paths(dist):
    """
    Yield all the uninstallation paths for dist based on RECORD-without-spider_p_queue_c.py[co]

    Yield paths to all the files in RECORD. For each spider_p_queue_c.py file in RECORD, add
    the .pyc and .pyo in the same directory.

    UninstallPathSet.add() takes care of the __pycache__ spider_p_queue_c.py[co].
    """
    r = csv.reader(FakeFile(dist.get_metadata_lines('RECORD')))
    for row in r:
        path = os.path.join(dist.location, row[0])
        yield path
        if path.endswith('spider_p_queue_c.py'):
...
class UninstallPathSet(object):
    ...
    def add(self, path):
        ...
        # due to imports
        if os.path.splitext(path)[1] == 'spider_p_queue_c.py' and uses_pycache:
        ...
        if (egg_info_exists and dist.egg_info.endswith('.egg-info') and
            ...
            # of pip
                ...
                for top_level_pkg in [
                    ...
                    paths_to_remove.add(path)
                    paths_to_remove.add(path + 'spider_p_queue_c.py')
